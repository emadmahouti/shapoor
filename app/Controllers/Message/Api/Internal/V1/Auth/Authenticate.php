<?php
namespace App\Controllers\Message\Api\Internal\V1\Auth;
...
class Authenticate extends Controller
{
    ...
    protected function verify()
    {
        ...
        if (is_null($phone) || is_null($code) || trim($phone) == "" || trim($code) == "")
            return $this->echoHttp(['status' => 'ورودی نامعتبر'], 401);
...
//                    return $this->echoHttp(['status' => 'اعتبار کد به پایان رسیده است'], 404);
                ...
                $_SESSION['user'] = "Bearer $jwt";

                return $this->echoNormal([
                    'status' => 'ok'
                ]);
            } else {
                return $this->echoHttp(['status' => 'کد اعتبار سنجی نادرست است'], 401);
            }
        }

        return $this->echoHttp(['status' => 'کاربر یافت نشد'], 401);
    }
}